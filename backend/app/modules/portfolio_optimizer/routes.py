...
def create(payload: PortfolioCreate, db: Session = Depends(get_db)):
    try:
...
def get(portfolio_id: int, db: Session = Depends(get_db)):
...
def delete(portfolio_id: int, db: Session = Depends(get_db)):
